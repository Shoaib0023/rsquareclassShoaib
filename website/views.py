...
def batches(request):
    ...
    return render(request, 'batch/batch_viewall.html', args)


def create_batch(request):
    if request.method == "POST":
        try:
            batch = request.POST.get('batchName').strip()
            academicYear = request.POST.get('academicYear').strip()

            batchObj = models.Batch.objects.create(batch_name=batch, academic_year=academicYear)
